...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    thisB = self.params['b' + str(self.num_layers)]
    fp_outputs['L' + str(self.num_layers)] = affine_forward(input, thisW, thisB)
    scores = fp_outputs['L' + str(self.num_layers)][0]
    ############################################################################
    ...
    for i in range(1, self.num_layers +1):
        thisW = self.params['W' + str(i)]
        sum_squared_W += np.sum(thisW**2)
    loss += 1/2.0 * self.reg * sum_squared_W
