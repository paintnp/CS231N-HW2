...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_norm = (x - sample_mean) * inv
    running_mean = momentum * running_mean + (1-momentum) * sample_mean
    running_var = momentum * running_var + (1-momentum) * sample_var


    out = gamma * x_norm + beta
    cache = (x, x_norm, sample_mean, sample_var, gamma, inv, eps)
    #############################################################################
...
def batchnorm_backward(dout, cache):
  ...
  x, x_norm, sample_mean, sample_var, gamma, inv, eps = cache
  m = x.shape[0]

  dgamma = np.sum(x_norm * dout, axis = 0) # (D,)
  dx_norm = gamma * dout # (N,D)
  dbeta = np.sum(dout, axis = 0)
  dx = inv * dx_norm # inv is (D,) dx = (N,D)
  dsample_mean =  np.sum(-inv * dx_norm, axis=0)
  dinv = np.sum((x-sample_mean) * dx_norm, axis=0)
  dsample_var = -0.5 * dinv * (sample_var + eps) ** (-1.5)
  dx += 1.0/m * 2.0 * (x - sample_mean) * dsample_var
  dsample_mean += 2.0/m * np.sum((x-sample_mean)*dsample_var, axis=0)
  dx += 1.0/m * dsample_mean
...
def max_pool_forward_naive(x, pool_param):
  ...
  for i in range(N):
      ...
      for c in range(C):
          for h in range(h_out):
              for wi in range(w_out):
                  input_chunk =  x_unpadded[c,h*stride:h*stride+HH, wi*stride:wi*stride+WW]
                  out[i,c,h,wi] = np.max(input_chunk)
  ...
  for i in range(N):
      ...
      for c in range(C):
          for h in range(h_out):
              for wi in range(w_out):
                  input_chunk =  x_unpadded[c,h*stride:h*stride+HH, wi*stride:wi*stride+WW]
                  max_index = np.argmax(input_chunk)
                  dx_chunk = dx_unpadded[c,h*stride:h*stride+HH, wi*stride:wi*stride+WW]
                  max_index_unravelled = np.unravel_index(max_index, (input_chunk.shape))
                  dx_chunk[max_index_unravelled] = dout[i,c,h,wi]
