...
def adam(x, dx, config=None):
  ...
  config.setdefault('mb', np.zeros_like(x))
  config.setdefault('mv', np.zeros_like(x))
  ...
  mb = config['m'] /(1-(beta1**(t+1)))
  mv = config['v']/(1-(beta2**(t+1)))
  config['t'] += 1
  next_x += -config['learning_rate'] * mb/(np.sqrt(mv) + config['epsilon'])
  #############################################################################
